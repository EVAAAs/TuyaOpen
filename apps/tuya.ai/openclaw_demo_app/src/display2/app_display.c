...
OPERATE_RET                   app_display_camera_start(uint16_t width, uint16_t height)
{
    ...
    for (uint8_t i = 0; i < 2; i++) {
        TUYA_CALL_ERR_RETURN(tdl_disp_fb_manage_add(sg_fb_manage, sg_disp_info.fmt, width, height));
    }
...
OPERATE_RET app_display_camera_flush(uint8_t *data, uint16_t width, uint16_t height)
{
    ...
    TUYA_CALL_ERR_LOG(tdl_disp_convert_yuv422_to_framebuffer(data, width, height, convert_fb));
