...
inline void ui_object_set_local_style_property(lv_obj_t *object_p, lv_style_selector_t selector,
                                               lv_style_prop_t property, ui_style_variable_t value)
{
...
// Atomic function to set (and register) an LVGL local style-property for a given part+state (selector) of an object
// (widget) - can be used in many ways due to being atomic
                                            ...
                                            const ui_theme_variable_t *theme_variable_p)
{
    ...
    lv_obj_set_local_style_prop(object_p, property,
                                _ui_style_value_convert(property, ui_get_theme_value(theme_variable_p)),
                                selector); // ui_object_set_local_style_property( object_p, selector, property,
                                           // ui_get_theme_value(theme_variable_p) );
}

// This function goes through all registered style-property settings and if theme is changed, sets all of them to the
// theme. (If called periodically, it can follow change of values automatically.)
void _ui_theme_set_variable_styles(uint8_t mode)
{
    ...
    for (i = 0; i < _ui_local_style_count; ++i) {
        ...
        if (style_variable_p != _ui_local_styles[i].previous_pointer ||
            style_value != _ui_local_styles[i].previous_value || mode == UI_VARIABLE_STYLES_MODE_INIT ||
            ui_Theme_Changed) {
            ...
            for (j = 0; j < _ui_local_styles[i].style_property_setting_count; ++j) {
                ...
                if (property_setting_p->next_p != NULL)
                    property_setting_p = (_ui_local_style_property_setting_t *)
                                             property_setting_p->next_p; // get next item in linked-list for next round
                else
...
lv_style_value_t _ui_style_value_convert(lv_style_prop_t property, ui_style_variable_t value)
{
    static lv_style_value_t
        Style_Value; // LVGL would produce artefacts if both .num and .color were set for the local style to add:
    // static lv_style_const_prop_t ValueConvert_Table [_LV_STYLE_NUM_BUILT_IN_PROPS] = { [LV_STYLE_BG_COLOR] =
    // LV_STYLE_CONST_BG_COLOR(1), [LV_STYLE_BG_OPA] = LV_STYLE_CONST_BG_OPA(1) };
    if (property == LV_STYLE_BG_COLOR || property == LV_STYLE_BG_GRAD_COLOR || property == LV_STYLE_BG_IMAGE_RECOLOR ||
        property == LV_STYLE_BORDER_COLOR || property == LV_STYLE_OUTLINE_COLOR || property == LV_STYLE_SHADOW_COLOR ||
        property == LV_STYLE_IMAGE_RECOLOR || property == LV_STYLE_LINE_COLOR || property == LV_STYLE_ARC_COLOR ||
...
_ui_local_style_t *_ui_local_style_create(const ui_style_variable_t *style_variable_p, bool is_themeable)
{
    ...
    _ui_local_styles =
        (_ui_local_style_t *)lv_realloc(_ui_local_styles, (_ui_local_style_count + 1) * sizeof(_ui_local_style_t));
...
// auto-update dynamic local style-array's 2nd dimension (object part+state style-property settings for a given
// style-variable)
                                                                            ...
                                                                            lv_style_prop_t     property)
{
    ...
    if (style_property_setting_p != NULL) { //checking might be unnecessary if count is properly initialized to 0
        if (local_style_p->garbage_collector_couter > 0) --local_style_p->garbage_collector_couter; //avoid calling
    garbage collector every time else if (LV_SQUARELINE_THEME__EXPLICIT_GARBAGE_COLLECTOR_PERIOD > 0) {
            ...
            for (i = 0; i < local_style_p->style_property_setting_count; ++i) {
                if (style_property_setting_p->object_p != NULL) { //check only valid object entry (skip already emptied
    positions{ #if ( defined(LV_SQUARELINE_THEME__OBJECT_VALIDITY_CACHE) && LV_SQUARELINE_THEME__OBJECT_VALIDITY_CACHE )
    //Not recommended inside explicit garbage collector. if ( style_property_setting_p->validcheck_counter > 0 )
    --style_property_setting_p->validcheck_counter; //avoids calling complex lv_obj_is_valid() many times else { //check
    objects with expired positive vaidity check if ( lv_obj_is_valid( style_property_setting_p->object_p ) )
    ...
    style_property_setting_p->object_p = NULL; //free up an entry of a deleted widget
                    ...
                    if ( !lv_obj_is_valid( style_property_setting_p->object_p ) ) style_property_setting_p->object_p =
    NULL; //free up an entry of a deleted widget #endif
                }
                if (style_property_setting_p->next_p != NULL) style_property_setting_p =
    (_ui_local_style_property_setting_t*) style_property_setting_p->next_p; //get next item in linked-list for next
        ...
        for (i = 0; i < local_style_p->style_property_setting_count; ++i) {
            if (empty_style_property_setting_p ==
                ...
                /*#ifdef LV_SQUARELINE_THEME__IMPLICIT_GARBAGE_COLLECTOR
                    ...
                    #if ( defined(LV_SQUARELINE_THEME__OBJECT_VALIDITY_CACHE) &&
                LV_SQUARELINE_THEME__OBJECT_VALIDITY_CACHE )  //for slow hardware, adding this and setting to 5..10 in
                lv_conf.h might make it faster if ( style_property_setting_p->validcheck_counter > 0 )
                --style_property_setting_p->validcheck_counter; //avoids calling complex lv_obj_is_valid() many times
                    else { //check objects with expired positive vaidity check
                        if ( lv_obj_is_valid( style_property_setting_p->object_p ) )
                style_property_setting_p->validcheck_counter = LV_SQUARELINE_THEME__OBJECT_VALIDITY_CACHE; else { //free
                up an entry of a deleted widget style_property_setting_p->object_p = NULL;
                            empty_style_property_setting_p = style_property_setting_p;
                        }
                    }
                    #else
                    if ( !lv_obj_is_valid( style_property_setting_p->object_p ) ) { //free up an entry of a deleted
                widget style_property_setting_p->object_p = NULL; empty_style_property_setting_p =
            ...
            if (style_property_setting_p->next_p != NULL)
                style_property_setting_p =
                    (_ui_local_style_property_setting_t *)
                        style_property_setting_p->next_p; // get next item in linked-list for next round
            else
    ...
    if (empty_style_property_setting_p == NULL) { // allocate new
        empty_style_property_setting_p =
            (_ui_local_style_property_setting_t *)lv_malloc(sizeof(_ui_local_style_property_setting_t));
    ...
    lv_obj_add_event_cb(object_p, _ui_local_style_property_setting_delete, LV_EVENT_DELETE,
