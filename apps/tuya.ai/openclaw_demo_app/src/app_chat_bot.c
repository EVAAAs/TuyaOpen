...
static void __ai_chat_handle_event(AI_NOTIFY_EVENT_T *event)
{
    ...
    case AI_USER_EVT_ASR_OK: {
        ...
        if (text != NULL && text->datalen > 0 && text->data != NULL) {
            uint16_t msgid = openclaw_remote_mqtt_send_user_text((const uint8_t *)text->data, (size_t)text->datalen,
                                                                 OPENCLAW_REMOTE_MQTT_QOS_AT_LEAST_ONCE);
            if (msgid > 0) {
                PR_INFO("User speech text sent to remote MQTT, msgid: %d, text: %.*s", msgid, (int)text->datalen,
                        text->data);
            } else {
                PR_ERR("Failed to send user speech text to remote MQTT");
            }
        }
    } break;
    default:
