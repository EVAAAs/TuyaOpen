...
OPERATE_RET ai_mode_free_register(void)
{
    ...
    AI_MODE_HANDLE_T handle = {
        .init         = __ai_mode_free_init,
        .deinit       = __ai_mode_free_deinit,
        .task         = __ai_mode_free_task,
        .handle_event = __ai_mode_free_handle_event,
        .get_state    = __ai_mode_free_get_state,
#if defined(ENABLE_BUTTON) && (ENABLE_BUTTON == 1)
        .handle_key   = __ai_mode_free_handle_key,
#endif
    };     
